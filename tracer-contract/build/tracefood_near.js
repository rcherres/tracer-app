...
const PAYMENT_AMOUNT_YOCTO = BigInt("100000000000000000000000");

// Define la clase principal del contrato.
let TraceFoodContract = (_dec = NearBindgen({}), _dec2 = initialize(), _dec3 = call({}), _dec4 = call({}), _dec5 = view(), _dec6 = view(), _dec(_class = (_class2 = class TraceFoodContract {
  ...
  init({
    ...
    log(`Contrato TraceFood inicializado con transiciones: ${JSON.stringify(stage_transitions)}`);
  }
  ...
  mint_lot({
    ...
    const farmerId = predecessorAccountId(); // El que llama es el agricultor
    ...
    const newLot = {
      ...
      events: [firstEvent],
      // Empieza con el evento de cosecha
    ...
    log(`Lote "${lot_id}" registrado por el agricultor "${farmerId}". Siguiente actor esperado: "${expectedNextActorId}".`);
  }
  ...
  confirm_stage({
    ...
    const callerId = predecessorAccountId(); // El que llama a esta función
    ...
    log(`Confirmando etapa "${stage_name}" para el lote "${lot_id}" por el actor "${callerId}".`);
    ...
    if (lot.expected_next_actor_id === null && lot.payment_status === "Pending") {
      log(`Etapa final "${stage_name}" confirmada para el lote "${lot_id}". Iniciando pago al agricultor "${lot.farmer_id}".`);
      // Crea una promesa para transferir tokens al agricultor.
      NearPromise.new(lot.farmer_id).transfer(PAYMENT_AMOUNT_YOCTO);
      ...
      log(`Siguiente actor esperado para el lote "${lot_id}" es "${lot.expected_next_actor_id}".`);
    } else {
      log(`Lote "${lot_id}" ya completó todas las etapas o el pago ya fue procesado.`);
    }
...
}, _applyDecoratedDescriptor(_class2.prototype, "init", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "init"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "mint_lot", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "mint_lot"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "confirm_stage", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "confirm_stage"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_lot_state", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "get_lot_state"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_all_lot_ids", [_dec6], Object.getOwnPropertyDescriptor(_class2.prototype, "get_all_lot_ids"), _class2.prototype), _class2)) || _class);
function get_all_lot_ids() {
  const _state = TraceFoodContract._getState();
  if (!_state && TraceFoodContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = TraceFoodContract._create();
  if (_state) {
    TraceFoodContract._reconstruct(_contract, _state);
  }
  const _args = TraceFoodContract._getArgs();
  const _result = _contract.get_all_lot_ids(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TraceFoodContract._serialize(_result, true));
}
function get_lot_state() {
  const _state = TraceFoodContract._getState();
  if (!_state && TraceFoodContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = TraceFoodContract._getArgs();
  const _result = _contract.get_lot_state(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TraceFoodContract._serialize(_result, true));
}
function confirm_stage() {
  ...
  const _args = TraceFoodContract._getArgs();
  const _result = _contract.confirm_stage(_args);
  TraceFoodContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TraceFoodContract._serialize(_result, true));
}
function mint_lot() {
  const _state = TraceFoodContract._getState();
  ...
  const _args = TraceFoodContract._getArgs();
  const _result = _contract.mint_lot(_args);
  TraceFoodContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TraceFoodContract._serialize(_result, true));
}
function init() {
...
export { confirm_stage, get_all_lot_ids, get_lot_state, init, mint_lot };
//# sourceMappingURL=tracefood_near.js.map
